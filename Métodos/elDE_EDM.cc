...
elDE_EDM::elDE_EDM(int func_num, int dimension, int population_size, double t_FES,int seed, std::string method_name) : elDE(func_num, dimension, population_size, 0, 0, t_FES, seed, method_name)
{
...
void elDE_EDM::selection(bool flag)
{
...
void elDE_EDM::updateDt()
{
    Dt = Di - Di * (1.1*FEs / maxFEs);
	Dt = ( Dt<=0 )? 0: Dt;
	if( FEs > (double) maxFEs/1.1) Dt = -1;
	else if( FEs > (double) maxFEs/1.05) Dt = -1;
};


void elDE_EDM::replacement()
{
    updateDt();
...
void elDE_EDM::setUnionToCurrent()
{
    //Copy Target
